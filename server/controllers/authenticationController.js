...
exports.signUp = catchAsync(async (req, res, next) => {
  ...
  if (!newUser) return new AppError('Something went wrong while signing up the user', 405);
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) return next(new AppError('You need to be logged in to access it', 401));
  // Verification of the token
  ...
  // Check if user changed password after jwt was issued
  if (currUser.changePasswordAfter(decoded.iat)) {
    return next(new AppError('the password was changed, login again to continue..', 401));
  }
 ...
 * restictTo
 * @param  {...any} roles
 * @returns error if role is not appropriate
 */
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError('No user found or the token is probably expired', 400));
